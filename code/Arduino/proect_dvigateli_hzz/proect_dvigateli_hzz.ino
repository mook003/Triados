...
int val = 0; //ввод какого-либо числа после ключа и дальнейшая обработка   
int val1 = 0;
...
void setup() {
  ...
  motorR.setMode(REVERSE); //если захочешь поменять направление вращения колес, меняй "AUTO" на "REVERSE"
  motorL.setMode(REVERSE);
}
...
if (Serial.available() > 1) {  //не совсем понял, что это значит
  ...
  val1 = Serial.parseFloat(); //ввод какого-либо числа после ключа и дальнейшая обработка   
