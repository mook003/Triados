...
`Max Range:` Точки, превышающие это значение (в м), использоваться не будут

`Point Subsample:` Характеризует выборку из полученных точек. Значение `1` означает, что все точки будут задействованы

`Padding Offset:` Размер отступа (в см)

`Padding Scale:` Масштаб заполнения

`Filtered Cloud Topic` Топик, по которому будет опубликовано отфильтрованное облако (для отладки) 

`Max Update Rate:` Частота, с которой карта будет обновляться
...
`octomap_frame:` :x: 
`octomap_resolution:` Задаёт разрешение карты (в м)
`max_range value:` Задаёт максимальное значение, оторое будет применяться для любого ввода датчика на эту ноду.
